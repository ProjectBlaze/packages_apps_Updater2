...
updates_dir=/data/blaze_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have blaze-VERSION-DATE-TYPE-* as format"
...
# Assume blaze-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.blaze.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.blaze.updater/databases/updates.db" \
